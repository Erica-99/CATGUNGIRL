...
@export var perfect_shot_handler: Node3D
...
func _ready() -> void:
	if perfect_shot_handler != null:
		perfect_shot_handler.perfect_shot_fired.connect()
...
func _update_aim(mouse_world: Vector3, input_state: Dictionary, delta: float) -> void:
	...
	gun._current_target_angle = target_angle
	...
	if gun._is_spamming and not gun._is_aim_settled():
		current_aim_speed = aim_speed * spam_aim_multiplier
		
	rotation.z = lerp_angle(rotation.z, target_angle + gun._recoil_offset + wobble, current_aim_speed * delta)
	gun._recoil_offset = lerpf(gun._recoil_offset, 0.0, recoil_recovery * delta)
	if abs(gun._recoil_offset) < 0.001:
		gun._recoil_offset = 0.0
